...
int main(int argc, char **argv)
{
	...
	if (rank == 0)
	{
		...
		unsigned int ultimas_filas = height % rowsPerProcess;
		currentRow = 0;

		for (currentWorker = 1; currentWorker < size && currentRow < height; currentWorker++)
		{
			rowsSentToWorker = rowsPerProcess;
			if (rowsSentToWorker > height - currentRow)
			{
				rowsSentToWorker = height - currentRow;
			}
		...
		while (devueltos < height)
		{
			...
			if (currentRow < height)
			{
				...
				MPI_Send(auxPtr, buffer_size, MPI_BYTE, status.MPI_SOURCE, tag, MPI_COMM_WORLD);
		...
		{
			MPI_Send(termination, 2 * sizeof(unsigned int), MPI_BYTE, worker, tag, MPI_COMM_WORLD);
		}
